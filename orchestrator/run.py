...
def жребий_вендоров(конфиг: dict, путь: Path,
    ...
    if избегать_повторов and прошлая:
        for попытка in range(1000):
            rng.shuffle(порядок)
            if all(в != прошлая.get(и) for и, в in zip(имена, порядок)):
                break
        else:
            raise SystemExit(
                "не удалось развести всех вендоров с прошлыми персонажами. "
                "Похоже, вариантов нет — проверьте «прошлая_раздача» в конфиге."
            )
    else:
        rng.shuffle(порядок)
    ...
    for имя, вендор in zip(имена, порядок):
        ...
        раздача[имя] = вендор
    # Запоминаем, чтобы следующий жребий было от чего разводить.
    ...
    print(f"— жребий вендоров (зерно {конфиг['зерно']})"
          + (", все меняют персонажа" if избегать_повторов and прошлая else "") + ":")
    for имя, вендор in раздача.items():
        было = прошлая.get(имя)
        пометка = f"   (было {было})" if было and было != вендор else ""
