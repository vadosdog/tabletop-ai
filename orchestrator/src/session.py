...
class Прогон:
    ...
    def _учесть(self, ответ: Ответ) -> None:
        ...
        for ключ, куда in соответствие:
            if isinstance(токены.get(ключ), int):
                self.токенов[куда] += токены[ключ]
    ...
    async def _круг(self) -> None:
        ...
        if self.боевка:
            ...
            # Своё состояние игрок видит цифрами, чужое — только со стороны.
            for имя in порядок:
                своё = self.боевка.сводка_игроку(имя)
                if своё:
                    self.доставить(имя, своё, "скрипт")
        self.журнал.записать(
    ...
    def _свод_ходов(self, ходы: list[Ход], шапка: str) -> str:
        явные = [х for х in ходы if not х.тайно]
