...
class Прогон:
    ...
    # Решимость снимает то, что давит на дух, а не на тело. Порез и слепота
    # силой воли не лечатся, а страх и оглушение — ровно то, против чего этот
    # ресурс в правилах и придуман.
    СНИМАЕТ_РЕШИМОСТЬ = ("сломлен", "оглушён")
    ...
    def _разобрать_траты(self, имя: str, текст: str) -> list[str]:
        ...
        for ресурс in parse.траты(текст):
            ...
            if ресурс == "судьба":
                # Судьбу тратят только по запросу скрипта при смертельном
                # ранении, а не по своей воле посреди хода.
                self._записать_ресурс({**основа, "подтверждено": False,
                                       "причина": "Судьба тратится только при "
                                                  "смертельном ранении, по запросу"})
                метки.append("заявка_отклонена")
                continue
    ...
    def _разобрать_решимость(self, текст: str) -> None:
        ...
        for запись in parse.начисления_решимости(текст, self.имена):
            ...
            self._записать_ресурс({
                ...
                "начислено": итог["добавлено"], "заявлено": True,
                "подтверждено": итог["успех"], "причина": запись["причина"],
                "отказ": итог["причина"], "было": итог["было"],
                "стало": итог["стало"],
            })
            if итог["успех"]:
                self.доставить(
    ...
    def _свести_ресурсы(self) -> dict:
        ...
        for событие in self.ресурсы_события:
            ...
            if событие.get("начислено"):
                строка["решимость_начислена"] += событие["начислено"]
                continue
